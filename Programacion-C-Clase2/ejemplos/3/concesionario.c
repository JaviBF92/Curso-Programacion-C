...
void curso_concesionario_attr_unset_coche(struct concesionario *con,
					  uint32_t pos)
{
	if (pos < 0 || pos > con->num_coches)	//corrección
		return;
				 ...
				 struct concesionario *con)
{
	...
	ret += snprintf(buf, size,
			...
			con->dueno, con->num_coches);	//snprintf devuelve la cantidad de memoria que ha sido ocupada 
							//ej: si se escribe hola, devuelve 4 (no 5, ya que solo cuenta los bytes efectivos)
	for (i = 0; i < con->num_coches; i++) {
		ret += curso_coche_snprintf(buf + ret, size - ret, con->garaje[i]);
		ret += snprintf(buf + ret, size - ret, "\n");
	}

	return ret;	//el ret se utiliza para controlar la cantidad de memoria que se ha escrito
}
