...
struct coche {
	...
	uint32_t	flags;
};	//hay dos grupos separados por ser el primero publico y el segundo privado(cod st)
...
{
	if (c->flags & (1 << CURSO_COCHE_ATTR_MARCA))	//<<:si CURSO... 2, desplaza 2 
		xfree(c->marca);
			  ...
			  uint32_t data_len)
{
	...
	case CURSO_COCHE_ATTR_MATRICULA:
		...
		c->matricula = strdup(data);//hace copia del puntero en otro lugar de mem
		break;
	case CURSO_COCHE_ATTR_ID:
		c->id = *((uint32_t *)data);
		break;
...
int curso_coche_snprintf(char *buf, size_t size, struct coche *c)
{
	...
	//escribe en la direccion de memoria indicada en buf, con el tamaño maximo size, la cadena a continuacion
}
